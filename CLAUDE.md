...
| `speaker-touch.html` | **Touch-friendly speaker-view layout.** Injected via `include-after-body` (with `pause-videos.html`). Adds a **"Touch"** layout to the reveal.js speaker-notes popup (the window opened with **S**): notes large on top, the **current slide full-width** below, **no upcoming-slide preview**, and **two big PREV / NEXT buttons** across the bottom for a touch screen (a slim clock/timer row is kept — a defense is timed). It's auto-selected when the popup opens and stays in the layout dropdown next to Default/Wide/Tall. **How:** the notes plugin opens the popup via `window.open('about:blank','reveal.js - Notes')` then `document.write`s the bundled `speaker-view.html`; this file wraps `window.open` in the *main* window, grabs the (same-origin) popup, injects a `<style>` for the `body[data-speaker-layout="touch"]` rule + the dropdown option + the nav bar, and wires the buttons to the **main** `window.Reveal.prev()/next()` (the plugin then syncs the popup's slide+notes). No edits to Quarto's install; inlined into the self-contained HTML. The `#touch-nav` UI lives only inside the popup, so it never appears on a slide, in the PDF/print export, or in the PPTX. |
...
Press **S** for speaker view (notes + timer + next-slide preview), **F** fullscreen, **O** overview, **B** blank/pause, **ESC** to exit. The speaker view has a layout dropdown (Default / Wide / Tall / Notes only) plus a custom **"Touch"** layout added by `speaker-touch.html` — big notes on top, full-width current slide below, and large PREV/NEXT buttons for a touch screen (auto-selected on open). Every content slide here carries a `.notes` block — keep them updated when slide content shifts.
